...
func TestHugoModulesMatrix(t *testing.T) {
	...
	if !htesting.IsCI() || neohugo.GoMinorVersion() < 12 {
...
`)
	...
	os.Mkdir(filepath.Join(workingDir, "mycontent"), 0o777)             // nolint
	os.Mkdir(filepath.Join(workingDir, "mycontent", "mybundle"), 0o777) // nolint
...
`
		...
		os.MkdirAll(filepath.Join(workingDir, "content", "blog"), 0o777) // nolint
...
`
		...
		os.MkdirAll(subContentDir, 0o777) // nolint
		myPartialsDir := filepath.Join(test.workingDir, "subdir", "mypartials")
		os.MkdirAll(myPartialsDir, 0o777) // nolint

		absShortcodesDir := filepath.Join(absDir, "abs", "myshortcodes")
		os.MkdirAll(absShortcodesDir, 0o777) // nolint
